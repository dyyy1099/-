        ...
        for seg_ind in indices:
            ...
            for i in range(self.duration):
                ...
                if p < record.num_frames - 1:
                    p += 1
        if "dfew" in video_frames_path[p]:
            video_folder_name = video_frames_path[p].split('/')[-2]  # 得到"00001"
            # 2. 去除前导零，转换为原始数字（如"00001"→1）
            raw_video_id = int(video_folder_name)  # 利用int自动去前导零，得到1
            # 3. 拼接音频路径（raw_wav/1.wav）
            audio_path = '/'.join(video_frames_path[p].split('/')[:-4]).replace('data', 'raw_wav') + '/' + str(raw_video_id) + '.wav'
            # 4. 加载音频
